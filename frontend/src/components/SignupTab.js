...
const SignupTab = ({ eventDetails, eventGuestData, userData, isConfirmedCancel, openDishSignupPopup }) => {   
    ...
    const fetchItems = async () => {
        try {
            const itemsResponse = await axios.get(`http://ec2-18-222-195-53.us-east-2.compute.amazonaws.com:8000/api/items/${eventDetails._id}`);
    ...
    const handleClick = (guest) => {
        ...
        .then(async () => {
            ...
            const res = await axios.get(`http://ec2-18-222-195-53.us-east-2.compute.amazonaws.com:8000/api/events/conversation/${eventDetails._id}/${meData.id}/${otherData.id}`);
